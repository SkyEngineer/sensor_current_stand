...
class MyWindow (QtWidgets.QMainWindow):
    ...
    def btn_connect_click(self):
        ...
        port_instek = self.ports.search_instek(port_stend)
        if port_instek != []:
            print(port_instek[0], port_instek[1])
        else:
            print("port_instek  не найден")

        port_measure_supply = self.ports.search_measure_supply(stand_port=port_stend)
        ...
        if port_stend != []:
            ...
            self.set_console_text("АППАРАТУРА НЕ НАЙДЕНА")
    ...
    def work_with_measure(self, measure_data):
        ...
        self.set_measure_data("u_supply_sens", str(int(measure_data[2])))
        self.set_measure_data("current_sens", str(int(measure_data[3])))
